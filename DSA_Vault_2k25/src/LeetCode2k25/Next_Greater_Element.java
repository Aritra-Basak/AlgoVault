...
public class Next_Greater_Element {
    ...
    public static int [] nextGreaterElements(int [] arr){
        ...
        for(int i=(2*n-1);i>=0;i--){
           while(!stack.isEmpty() && arr[i % n]>=stack.peek()){ // if stack is full and the top element is smaller than the array's current element then keep on removing the elements from stack
               stack.pop();
           }
           if(i<n){ //take the top element from the stack
               if(!stack.isEmpty())
                   nge[i]=stack.peek();
               else
                   nge[i]=-1;
           }
           stack.push(arr[i%n]); //keep on pushing the elements.
        }
        return nge;
    }
}
